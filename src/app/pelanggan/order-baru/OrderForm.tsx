...
export function OrderForm({
  ...
  const redeemAktif = Math.max(0, Math.min(redeemInput, saldoLoyalti, total));
  const totalSetelahRedeem = Math.max(0, total - redeemAktif);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 pb-32 sm:pb-4">
      ...
      {saldoLoyalti > 0 && total > 0 && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 space-y-2">
          <div className="font-bold text-emerald-900 text-sm inline-flex items-center gap-1.5">
            💎 Bayar pakai Saldo Loyalty
          </div>
          <div className="text-xs text-emerald-800">
            Saldo Anda: <b>{formatRupiah(saldoLoyalti)}</b>
          </div>
          <div className="flex gap-2 items-center">
            <input
              type="number"
              min={0}
              max={Math.min(saldoLoyalti, total)}
              value={redeemInput || ""}
              onChange={(e) => setRedeemInput(Math.max(0, Number(e.target.value)))}
              placeholder="Jumlah dipakai"
              className="flex-1 px-3 py-2 border border-emerald-300 rounded-lg text-sm bg-white"
            />
            <button
              type="button"
              onClick={() => setRedeemInput(Math.min(saldoLoyalti, total))}
          ...
          {redeemAktif > 0 && (
            <div className="text-xs font-bold text-emerald-800">
              {redeemAktif >= total
                ? "✓ Saldo cukup — order langsung LUNAS tanpa perlu bayar online!"
                : `Sisa yang harus dibayar: ${formatRupiah(totalSetelahRedeem)}`}
            </div>
          )}
        </div>
      )}
            ...
            <div>
              ...
              {redeemAktif > 0 && (
                <div className="text-xs text-emerald-700 font-bold">
                  💎 Loyalti -{formatRupiah(redeemAktif)} → Bayar {formatRupiah(totalSetelahRedeem)}
                  {totalSetelahRedeem === 0 && " (GRATIS!)"}
                </div>
              )}
            </div>
          </div>
